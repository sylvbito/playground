import { mix, readableForeground } from './color.js?v=codex-2';

const editorInk = (colour, background) => readableForeground(colour, background, 4.5);
function editorDiffInk(colour, background) {
  let repaired = editorInk(colour, background);
  for (let index = 0; index < 3; index += 1) repaired = readableForeground(repaired, mix(background, repaired, 0.14), 4.5);
  return repaired;
}
...
export function applyPlatformAppearance(root, tokens, theme, settings, variant) {
  ...
  root.style.setProperty('--codex-base-accent', theme.accent);
  root.style.setProperty('--codex-base-surface', theme.surface);
  root.style.setProperty('--codex-base-ink', theme.ink);
  root.style.setProperty('--codex-base-contrast', String(theme.contrast));
...
export function applyAppearance(root, tokens, theme, settings, variant, editorTheme) {
  ...
  root.classList.toggle('marker-symbols', settings.diffMarkerStyle === 'symbols');
